...
public class Leaderboard {
	...
	public static List<Map<Key, Object>> getData(int count, int page) {
		try {
			...
			for (int i = count * (page - 1); i < count * page && i < rawData.size(); i++) {
				...
				int index = i;
				data.add(new HashMap<Key, Object>() {{
					put(Key.RANK, index + 1);
					put(Key.TAG, nextRow.get(1).text().replaceFirst("\\d+?\\. ", ""));
					put(Key.LEVEL, Long.parseLong(nextRow.get(2).text()));
					put(Key.EXP, Long.parseLong(nextRow.get(3).text()));
					put(Key.UP_TO, Leaderboard.getExpUpTo((long) get(Key.LEVEL)));
					put(Key.TOTAL_XP, Leaderboard.getTotalExp((long) get(Key.LEVEL), (long) get(Key.EXP)));
				}});
			}
	...
	public static List<Map<Key, Object>> getData() {
		try {
			List<Map<Key, Object>> result = new ArrayList<>();
			Elements data = Jsoup.connect(LEADERBOARD_URL).timeout(5000)
					.get().getElementsByClass("table table-dark mx-auto").select("tbody").select("tr");

			for (int i = 0; i < data.size(); i++) {
				Elements next = data.get(i).select("td");
				Map<Key, Object> eachData = new HashMap<>();
				eachData.put(Key.RANK, i + 1);
				eachData.put(Key.TAG, next.get(1).text().replaceFirst("\\d+?\\. ", ""));
				eachData.put(Key.LEVEL, Long.parseLong(next.get(2).text()));
				eachData.put(Key.EXP, Long.parseLong(next.get(3).text()));
				eachData.put(Key.UP_TO, Leaderboard.getExpUpTo((long) eachData.get(Key.LEVEL)));
				eachData.put(Key.TOTAL_XP, Leaderboard.getTotalExp((long) eachData.get(Key.LEVEL), (long) eachData.get(Key.EXP)));
				result.add(eachData);
			}
	...
	public static Map<Key, Object> getData(String tag) {
		return getData().stream().findFirst().filter($ -> $.get(Key.TAG).equals(tag.replaceFirst("(?<=#\\d{2})\\d{2}$", "**"))).get();
	}
